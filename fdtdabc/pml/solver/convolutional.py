...
class Solver(SolverBase):
    ...
    def __init__(self, num_pml_cells_left, num_pml_cells_right, sigma_kappa_order = 4, sigma_max_ratio = 0.75, kappa_max = np.array([15.0, 15.0, 15.0]), alpha_order = 1.0, alpha_max = np.array([0.24, 0.24, 0.24])):
    ...
    def _init_coeffs(self, grid, dt):
        ...
        sigma_max = get_sigma_max(grid.steps, self._sigma_kappa_order, self._sigma_max_ratio)
    ...
    def _update_pml_e_element(self, grid, dt, i, j, k, dbx_dy, dbx_dz, dby_dx, dby_dz, dbz_dx, dbz_dy):
        ...
        psi_e = self._e_split
        # Taflove 3rd ed. (7.105), compared to that our psi_e values use B instead of H
        # and so have a factor mu0
        ...
        # Taflove 3rd ed. (7.106), divide by mu0 as H = B / mu0 and psi_e are also scaled by mu0
    ...
    def _update_pml_b_element(self, grid, dt, i, j, k, dex_dy, dex_dz, dey_dx, dey_dz, dez_dx, dez_dy):
        ...
        psi_b = self._b_split
        # Taflove 3rd ed. (7.110), our psi_b values are the same values as psi_h there,
        # which is compensated for later
        ...
        # Taflove 3rd ed. (7.108), both sides muptiplied by mu0
